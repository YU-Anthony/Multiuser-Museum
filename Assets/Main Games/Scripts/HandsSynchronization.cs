...
public class HandsSynchronization : MonoBehaviour, IPunObservable
{
    ...
    {
        // PhotonStream is a contained class that sends and receives data from and to a Photonview
        // If the photonView is mine, I am the one who controls this player and I am the one who moves the VR hands
        // So, as I move the controllers, I should send my controllers' transform data to the other players' game;
        // So that the other players can update my hand controllers transform in their game
        if (stream.IsWriting)
        {
            //The player is me
        ...
        else
        {
            ...
            // Because we need this check only initially
            if (m_firstTake)
            {
                m_firstTake = false;
            }
